...
| `ADMIN_AUTH_ENABLED` | Web 登录：非本机监听（`0.0.0.0` / `::`）时必须设为 `true`，并在启动前执行 `python -m src.auth reset_password` 设置初始密码，否则服务拒绝启动；仅本机监听可在首次访问时设置密码。可在「系统设置 > 修改密码」修改；忘记密码同样执行该 CLI。Web 的 `.env` 备份导入导出仅在开启该开关后可用（桌面端不受此限制）。 | `false` |
