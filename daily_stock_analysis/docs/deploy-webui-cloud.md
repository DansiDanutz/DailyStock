...
把 `127.0.0.1` 改成 `0.0.0.0`，并同时启用管理员认证：
...
> `127.0.0.1` 表示只有本机能访问，`0.0.0.0` 表示允许任何来源访问。云服务器需要把 `.env` 中的 `WEBUI_HOST` 改成 `0.0.0.0`，或在启动命令里显式传入 `--host 0.0.0.0`，并启用管理员认证；缺少认证时服务会拒绝非本机监听。
...
Web 界面绑定到非本机地址前必须开启登录密码保护；否则服务会拒绝启动：
